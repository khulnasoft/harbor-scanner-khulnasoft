...
func TestCommand_Scan(t *testing.T) {
	...
	config := etc.KhulnasoftCSP{
		ReportsDir:                            "/var/lib/reports",
		Username:                              "scanner",
		Password:                              "ch@ng3me!",
		Host:                                  "https://khulnasoft.domain:8080",
		Registry:                              "Harbor",
		UseImageTag:                           true,
	...
	t.Run("Should return error when creating tmp file returns error", func(t *testing.T) {
		...
		ambassador.On("TempFile", config.ReportsDir, "khulnasoft_scan_report_*.json").
	...
	t.Run("Should return error when running scannercli command returns error", func(t *testing.T) {
		...
		ambassador.On("LookPath", "scannercli").
			Return("/usr/local/bin/scannercli", NoError)
		ambassador.On("TempFile", config.ReportsDir, "khulnasoft_scan_report_*.json").
			Return(ext.NewFakeFile("/var/lib/scanner/reports/khulnasoft_scan_report_1234567890.json", strings.NewReader("{}")), NoError)
		ambassador.On("Remove", "/var/lib/scanner/reports/khulnasoft_scan_report_1234567890.json").
			Return(NoError)
		ambassador.On("RunCmd", &exec.Cmd{
			...
			Args: []string{
				...
				"--jsonfile=/var/lib/scanner/reports/khulnasoft_scan_report_1234567890.json",
				"--register-compliant",
	...
	*/
	t.Run("Should store the tmp report file", func(t *testing.T) {
		khulnasoftReportJSON, err := os.Open("test_fixtures/khulnasoft_report_photon_3.0.json")
		require.NoError(t, err)
		defer func() {
			_ = khulnasoftReportJSON.Close()
		}()
		...
		ambassador.On("LookPath", "scannercli").
			Return("/usr/local/bin/scannercli", NoError)
		ambassador.On("TempFile", config.ReportsDir, "khulnasoft_scan_report_*.json").
			Return(ext.NewFakeFile("/var/lib/scanner/reports/khulnasoft_scan_report_1234567890.json", khulnasoftReportJSON), NoError)
		//		ambassador.On("Remove", "/var/lib/scanner/reports/khulnasoft_scan_report_1234567890.json").
		//			Return(NoError)
		ambassador.On("RunCmd", &exec.Cmd{
			Path: "/usr/local/bin/scannercli",
		...
		khulnasoftReport, err := NewCommand(config, ambassador).Scan(imageRef)
		require.NoError(t, err)
					...
					Vulnerabilities: []Vulnerability{
						{
							...
							KhulnasoftVectors:       "CVSS:3.0/AV:L/AC:L/PR:L/UI:N/S:U/C:H/I:H/A:H",
							KhulnasoftScoringSystem: "CVSS V3",
						},
						{
							Name:              "CVE-2019-18276",
							...
							KhulnasoftScore:         7.8,
							KhulnasoftSeverity:      "high",
							KhulnasoftVectors:       "CVSS:3.1/AV:L/AC:L/PR:L/UI:N/S:U/C:H/I:H/A:H",
							KhulnasoftScoringSystem: "CVSS V3",
						},
					},
				},
					...
					Vulnerabilities: []Vulnerability{
						{
							...
							KhulnasoftVectors:       "CVSS:3.0/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:H",
							KhulnasoftScoringSystem: "CVSS V3",
						},
					},
				},
